...
<?php include('../layout/admin/menu.php');?>
  ...
  <div class="container">
        ...
        <?php
        $id_get = $_GET['id'];
        $query_rol = $pdo->prepare("SELECT * FROM tb_eventos WHERE id_evento = '$id_get' AND  estado = '1' ");
        $query_rol->execute();
        $roles = $query_rol->fetchAll(PDO::FETCH_ASSOC);
        foreach($roles as $rol){
            $id = $rol['id_evento'];
            $nombre = $rol['nombre_evento'];
        }
        ?>
            ...
            <div class="col-md-6">
                <div class="card" style="border: 1px solid #AD52FE">
                    <div class="card-header" style="background-color: #D8ADFE">
                        <h4>¿Estás seguro de eliminar el Evento?</h4>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <a href="<?php echo $URL;?>/salones/mapeo.php" class="btn btn-default" >Cancelar</a>
                        </div>
                        <div id="respuesta">
...
<script>
    $('#btnborrar').click(function () {
        var id_evento = <?php echo $id_get = $_GET['id'];?>

            var url="controller_delete.php";
            $.get(url,{id_evento:id_evento},function(datos){
